...
import io.hyperfoil.tools.pipelineManager.api.IRuntimeEnvironment;
...
public class JenkinsService implements IRuntimeEnvironment {
    ...
    @ConfigProperty( name = "pipeline-manager.jenkins.url")
    public String JENKINS_URL;

    @ConfigProperty( name = "pipeline-manager.jenkins.username")
    public String USERNAME;

    @ConfigProperty( name = "pipeline-manager.jenkins.password")
...
/*
    public String newRun(ExperimentDAO pipelineDAO, TrialConfig trialConfig) {
        connectToJenkins();
        */
/*
         * TODO:: Either need to;
        ...
        Map<String, String> jobParams = trialConfig.tunableConfigs().stream()
                .filter(tunableConfig -> pipelineDAO.jenkins.params.containsKey(tunableConfig.name()))
                .collect(Collectors.toMap(tuneable -> pipelineDAO.jenkins.params.get(tuneable.name()), tuneable -> tuneable.value().toString()));
        ...
        try {
            ...
            JobWithDetails job = jenkins.getJob(pipelineDAO.jenkins.job_url);
            if ( job == null){
                return "Could not find job: ".concat(pipelineDAO.jenkins.job_url);
            }
            job.build(jobParams);
